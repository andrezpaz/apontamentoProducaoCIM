...
def insert_varibles_into_table(recurso, etapa, seq_fila, op, cod_item, desc_item, cod_clicheria, iniprog, fimprog, mrp):
    try:
        connection = create_server_connection()
        cursor = connection.cursor()
        ...
        record = (recurso, etapa, seq_fila, op, cod_item, desc_item, cod_clicheria, iniprog, fimprog, mrp)
        print(record)
...
with open ('./import/CIMINIFLEX.csv', newline='', encoding='latin-1') as csvfile:
    ...
    for row in fila:
        ...
        insert_varibles_into_table(recurso, etapa, seq_fila, op, cod_item, desc_item, cod_clicheria, 
                                   ini_prog_format, fim_prog_format, numeroMRP) ## Inicia insert no banco, de forma individual
